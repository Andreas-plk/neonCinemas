    ...
    return (
        <div className="movie-card">
            <Link href={`/movie/${movie.Id}`} >
                <img src={movie.backdropPath!==null ?(`https://image.tmdb.org/t/p/original/${movie.backdropPath}`):("/posterNotFound.png")} alt="Movie cover"
            ...
            <div className="flex flex-col-2 w-full justify-between  text-xs text-text/80 mb-3">
                <p>
                    {movie.releaseDate && new Date(movie.releaseDate).getFullYear()} • {movie.genres.slice(0,2).map(g => g.name).join(" • ")}
                </p>
                <p>
                    {movie.runtime && MinToHours(movie.runtime)}
                </p>
